...
- `betType` 目前支援 `correct_score`、`half_time_correct_score`、`match_winner`、`tournament_champion`。正確比分與上半場波膽必須拆成主隊、客隊兩個數值欄位；其他玩法以 `selection` 儲存選擇，不能把所有玩法硬轉成比分。
...
- 正確比分與全場獨贏只判定正規時間比分／勝平負，不包含延長賽與 PK 大戰；上半場波膽只依 ESPN 可驗證的上半場比分結算；冠軍單在決賽正式完成後，以 ESPN 標示的晉級／勝方判定。
